 ...
 * java.sql.SQLException; import java.util.logging.Logger;
 * 
 ...
 * 
 * import DB.DBConn; import DB.DBQueries; import javafx.scene.control.Alert;
 * import javafx.scene.control.Alert.AlertType;
 * 
 * public class Katalog {
 * 
 * private static final Logger log; static DBQueries dBQuery = new DBQueries();
 * 
 * static { System.setProperty("java.util.logging.SimpleFormatter.format",
 * "[%4$-7s] %5$s %n"); log = Logger.getLogger(DBConn.class.getName()); }
 * 
 * String Bezeichnung;
 * 
 ...
 * 
 * 
 * public static void exportToExcel(String fileName) throws SQLException { //
 * fileName = Name of the katalog.xlsx
 * 
 * try {
 * 
 * // TO-DO: Send query method! dBQuery.alleFrageLaden();
 * 
 * // Create Excel Workbook object XSSFWorkbook workbook = new XSSFWorkbook();
 * // Create Excel Sheet in this Workbook XSSFSheet sheet =
 * workbook.createSheet("Fragen"); // Create Header in this sheet XSSFRow
 * kopfzeile = sheet.createRow(0); // Create structure of the header in the
 * sheet kopfzeile.createCell(0).setCellValue("ID");
 ...
 * kopfzeile.createCell(8).setCellValue("Fragekatalog"); // Fragekatalog becomes
 * redundant with this solution
 * 
 ...
 * 
 * int ID = DBQueries.rs.getInt("idFrage"); String thema =
 ...
 * 
 * XSSFRow zeile = sheet.createRow(index); zeile.createCell(0).setCellValue(ID);
 ...
 * File file = new File("Kataloge/" + fileName + ".xlsx"); FileOutputStream
 * fileout = new FileOutputStream(file); workbook.write(fileout);
 * workbook.close(); fileout.close();
 * 
 * Alert alert = new Alert(AlertType.INFORMATION); alert.setTitle("");
 * alert.setHeaderText(null);
 * alert.setContentText("Katalog erfolgreich erstellt"); alert.showAndWait();
 * 
 * } catch (IOException e) {
 * 
